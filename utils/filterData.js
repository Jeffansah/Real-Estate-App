...
const convertCurrency = (value) => {
  ...
  const convertedValue =
    Math.floor((parseFloat(value) * conversionRate) / 1000) * 1000;
  return {
    value: convertedValue.toString(),
    name: convertedValue.toLocaleString(),
  };
};

filterData.forEach((filter) => {
  if (filter.queryName === "minPrice" || filter.queryName === "maxPrice") {
    filter.items.forEach((item) => {
      const convertedItem = convertCurrency(item.value);
      item.value = convertedItem.value;
